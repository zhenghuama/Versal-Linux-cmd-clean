...
template <int M_API, int K_API, int N_API, int single_M, int single_K, int single_N, int SHIFT, int PART>
void partial_gemm(
	input_window_int8 * __restrict matA,
	output_window_int8 * __restrict matC, const int8 matB []) {
...
//		Just write it this way, don't question it, or it won't scheudle at every clk.
		{
			const int8 * __restrict pA1 = pA + (PART * single_K) // Parallel offset (columns)
				+ ( i * (single_K/K_API) + 0) * MMUL::size_A;
			const int8 * __restrict pA2 = pA + (PART * single_K) // Parallel offset (columns)
				+( (i+1) * (single_K/K_API) + 0) * MMUL::size_A;
